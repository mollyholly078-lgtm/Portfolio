  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-lg animate-slide-up">
        ...
        <p className="text-text-muted text-center mb-6">Catkey - Here's how everyone did</p>

        <div className="space-y-3 mb-6">
          {sorted.map((player, i) => (
            <div key={player.id}
              className={`flex items-center gap-3 p-4 rounded-xl ${i === 0 ? 'bg-accent/10 ring-1 ring-accent/30' : 'bg-surface'} animate-slide-up`}
              ...
              <span className="text-2xl font-bold font-mono" style={{ color: player.color }}>{player.score.toFixed(1)}</span>
            </div>
          ))}
        </div>
            ...
            <div className="space-y-1.5">
              {histories.map((h, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-text-muted text-xs">({h.category})</span>
        ...
        {!isHost && (
          <p className="text-center text-text-muted text-sm">Waiting for host to start a new game...</p>
        )}
      </div>
    </div>
  )
